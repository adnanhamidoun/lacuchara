...
const CUISINE_OPTIONS: Array<{ value: CuisineOption; label: string }> = [
  ...
  {
    value: "grill",
    label: `${CUISINE_META.grill.emoji} ${CUISINE_META.grill.label}`,
  },
  {
    value: "spanish",
    label: `${CUISINE_META.spanish.emoji} ${CUISINE_META.spanish.label}`,
  },
  {
    value: "mediterranean",
    label: `${CUISINE_META.mediterranean.emoji} ${CUISINE_META.mediterranean.label}`,
  },
  {
    value: "stew",
    label: `${CUISINE_META.stew.emoji} ${CUISINE_META.stew.label}`,
  },
  {
    value: "fried",
    label: `${CUISINE_META.fried.emoji} ${CUISINE_META.fried.label}`,
  },
  {
    value: "italian",
    label: `${CUISINE_META.italian.emoji} ${CUISINE_META.italian.label}`,
  },
  {
    value: "asian",
    label: `${CUISINE_META.asian.emoji} ${CUISINE_META.asian.label}`,
  },
  {
    value: "latin",
    label: `${CUISINE_META.latin.emoji} ${CUISINE_META.latin.label}`,
  },
  {
    value: "arabic",
    label: `${CUISINE_META.arabic.emoji} ${CUISINE_META.arabic.label}`,
  },
  {
    value: "avantgarde",
    label: `${CUISINE_META.avantgarde.emoji} ${CUISINE_META.avantgarde.label}`,
  },
  {
    value: "plantbased",
    label: `${CUISINE_META.plantbased.emoji} ${CUISINE_META.plantbased.label}`,
  },
  {
    value: "streetfood",
    label: `${CUISINE_META.streetfood.emoji} ${CUISINE_META.streetfood.label}`,
  },
];
...
function normalizeTerraceOption(
  value: string | null | undefined,
): TerraceOption {
  const normalized = (value ?? "").trim().toLowerCase();
  if (!normalized) return "";
...
function normalizeCuisineOption(
  value: string | null | undefined,
): CuisineOption {
  const normalized = (value ?? "").trim().toLowerCase();
  if (!normalized) return "";
  ...
  if (normalized === "mediterránea" || normalized === "mediterranea")
...
function mapRestaurantToForm(detail: RestaurantDetail): ProfileFormState {
  return {
    ...
    opens_weekends:
      detail.opens_weekends === null
        ? ""
        : detail.opens_weekends
          ? "true"
          : "false",
    has_wifi:
      detail.has_wifi === null ? "" : detail.has_wifi ? "true" : "false",
...
export default function RestaurantPanelView() {
    ...
    const loadRestaurantAssets = async () => {
      ...
      try {
        const [detail, imageData] = await Promise.all([
          ...
          getRestaurantImage(session.restaurant_id).catch(() => ({
            image_url: "https://placehold.co/400x240?text=Restaurante",
          })),
        ]);
        ...
        setPreviewUrl(
          imageData.image_url ||
            "https://placehold.co/400x240?text=Restaurante",
        );
      } catch (err) {
        setProfileError(
          err instanceof Error
            ? err.message
            : "No se pudo cargar la ficha del restaurante.",
        );
    ...
    const loadTodayMenu = async () => {
      ...
      try {
        const response = await fetch(
          `/restaurants/${session.restaurant_id}/menu/today`,
        );
        if (!response.ok) {
        ...
        setOcrMessage(
          "Menú de hoy cargado. Puedes editarlo y volver a publicarlo.",
        );
        setOcrError("");
      } catch {
        setHasTodayMenu(false);
  ...
  const setProfileField = <K extends keyof ProfileFormState>(
    key: K,
  ...
  const handleSaveProfile = async () => {
    ...
    for (const field of numericFields) {
      ...
      if (
        ...
        (typeof field.max === "number" && parsedValue > field.max)
      ) {
        setProfileError(
    ...
    const payload: RestaurantUpdatePayload = {
      ...
      min_service_duration: stringToNullableNumber(
        profileForm.min_service_duration,
      ),
      terrace_setup_type:
        normalizeTerraceOption(profileForm.terrace_setup_type) || null,
      opens_weekends: stringToNullableBoolean(profileForm.opens_weekends),
      has_wifi: stringToNullableBoolean(profileForm.has_wifi),
      restaurant_segment:
        normalizeSegmentOption(profileForm.restaurant_segment) || null,
      menu_price: stringToNullableNumber(profileForm.menu_price),
      dist_office_towers: stringToNullableNumber(
        profileForm.dist_office_towers,
      ),
    ...
    try {
      ...
      setProfileError(
        err instanceof Error
          ? err.message
          : "No se pudo guardar la ficha del restaurante.",
      );
    } finally {
  ...
  const handleOcrUpload = async () => {
    ...
    try {
      ...
      setMenuData((current) => ({
        starter: menu?.starter_options?.length
          ? menu.starter_options.join("; ")
          : menu?.starter || "",
        main: menu?.main_options?.length
          ? menu.main_options.join("; ")
          : menu?.main || "",
        dessert: menu?.dessert_options?.length
          ? menu.dessert_options.join("; ")
          : menu?.dessert || "",
      ...
      setOcrError(
        err instanceof Error ? err.message : "Error en lectura de IA",
      );
    } finally {
      setOcrLoading(false);
    }
  ...
  return (
    <section className="space-y-8">
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-lg">
              <div className="mb-6">
                ...
                <p className="text-sm text-[var(--text-muted)] mt-1">
                  Información pública de tu restaurante para los algoritmos de
                  predicción.
                </p>
              </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setProfileField("capacity_limit", event.target.value)
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)]/50 outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                      placeholder="Ej: 80"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setProfileField("table_count", event.target.value)
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)]/50 outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                      placeholder="Ej: 16"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setProfileField("menu_price", event.target.value)
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)]/50 outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setProfileField("google_rating", event.target.value)
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)]/50 outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                      placeholder="Ej: 4.3 ⭐"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProfileField(
                          "cuisine_type",
                          event.target.value as CuisineOption,
                        )
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProfileField(
                          "restaurant_segment",
                          event.target.value as SegmentOption,
                        )
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProfileField(
                          "terrace_setup_type",
                          event.target.value as TerraceOption,
                        )
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProfileField(
                          "opens_weekends",
                          event.target.value as NullableBooleanValue,
                        )
                      }
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20"
                    >
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-lg overflow-hidden">
              ...
              <p className="text-xs text-[var(--text-muted)] mt-4">
                {restaurantDetail?.name &&
                  `${restaurantDetail.name} �?� ${restaurantDetail.cuisine_type}`}
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === "ocr" && (
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#E07B54] text-white text-sm font-bold">
                  1
                </span>
              ...
              {loadingTodayMenu ? (
                ...
                <div className="mb-6 rounded-xl bg-[#FF9800]/10 border border-[#FF9800]/30 p-4 text-sm text-[#FF9800]">
                  <div className="flex gap-3">
                    <span>�"�️</span>
                    <span>
                      Ya existe un menú de hoy. Si subes uno nuevo, reemplazará
                      los datos.
                    </span>
                  </div>
                </div>
              ...
              <label className="relative block cursor-pointer">
                ...
                <div className="rounded-2xl border-2 border-dashed border-[var(--border)] bg-[var(--surface-soft)]/30 p-12 text-center transition-all hover:border-[#E07B54] hover:bg-[var(--surface-soft)]/60">
                  <div className="space-y-4">
                    <div className="text-5xl inline-block">�Y"�</div>
                    <div>
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[var(--border)] text-[var(--text)] text-sm font-bold">
                  2
                </span>
                <h2 className="text-2xl font-bold text-[var(--text)]">
                  Revisa y publica
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-lg space-y-6">
              ...
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setMenuData({ ...menuData, starter: event.target.value })
                  }
              ...
              <div className="space-y-3">
                ...
                <textarea
                  value={menuData.main}
                  onChange={(event) =>
                    setMenuData({ ...menuData, main: event.target.value })
                  }
                  className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20 resize-none"
                  rows={2}
              ...
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setMenuData({ ...menuData, dessert: event.target.value })
                  }
                  className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-3 text-sm text-[var(--text)] outline-none transition-all focus:border-[#E07B54] focus:ring-1 focus:ring-[#E07B54]/20 resize-none"
                  rows={2}
              ...
              <label className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)]/40 px-4 py-3 cursor-pointer hover:bg-[var(--surface-soft)]/60 transition-all">
                <input
                  ...
                  onChange={(event) =>
                    setMenuData({
                      ...menuData,
                      includes_drink: event.target.checked,
                    })
                  }
      ...
      {activeTab === "predicciones" && <PredictionDashboard />}
    </section>
  );
}
