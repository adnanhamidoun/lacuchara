...
export default function AITransparencyCard({
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-lg">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">
            Modelo impulsado por IA - Información de transparencia
          </p>
        </div>
      </div>
      ...
      {/* Confidence Badge */}
      <div
        className={`inline-block rounded-lg border px-3 py-1.5 text-xs font-semibold mb-6 ${confidenceColors[confidenceLevel]}`}
      >
      ...
      <div className="mb-6">
        ...
        <ul className="space-y-2">
          {dataUsed.map((item, idx) => (
            ...
            >
              <span className="text-[#E07B54] mt-1">�o"</span>
              <span>{item}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="pt-6 border-t border-[var(--border)]">
        <p className="text-xs text-[var(--text-muted)] flex items-center justify-between">
          <span>
            Esta predicción fue generada por un modelo de aprendizaje automático
          </span>
