      ...
      {[0, 1, 2, 3, 4].map((i) => (
        <div key={i} className="relative h-4 w-4 overflow-hidden">
          ...
          >
            <Star
              size={16}
              className="absolute fill-[#E07B54] text-[#E07B54]"
            />
          </div>
        </div>
      ))}
...
const FeaturedRestaurantCard = memo(function FeaturedRestaurantCard({
  ...
  const [imageUrl, setImageUrl] = useState<string>(
    "https://placehold.co/400x300?text=Restaurante",
  );
    ...
    const loadImage = async () => {
      try {
        const response = await fetch(
          `/get-restaurant-image/${restaurant.restaurant_id}`,
        );
        if (response.ok) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <h3 className="line-clamp-2 text-lg font-bold text-[var(--text)]">
          {restaurant.name}
        </h3>
