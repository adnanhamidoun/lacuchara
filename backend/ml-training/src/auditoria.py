...
def realizar_auditoria(df):
    print("�Y"� STARTING VIGARDO 2.0 AUDIT...\n")
    ...
    print(f"�Y�~ Ratio de Paella los Jueves: {ratio_paella:.2%}")

    # 2. Test del Calor (Temp > 25ºC y dishes calientes)
    dishes_calientes = ['Lentejas estofadas', 'Sopa castellana', 'Potaje de garbanzos', 'Minestrone', 'Sopa de pescado']
    dias_calor = df[df['max_temp_c'] > 25]
    errores_calor = dias_calor[dias_calor['first_course'].isin(dishes_calientes)]
    ratio_error_calor = len(errores_calor) / len(dias_calor) if len(dias_calor) > 0 else 0
    print(f"�Y"� 'Crimen': Platos calientes con calor (>25ºC): {ratio_error_calor:.2%}")

    # 3. Test de Identidad (¿Se respetan las cocinas?)
    # Check whether an Italian restaurant serves something not in its config (e.g., lentils)
    errores_identidad = df[(df['cuisine_type'] == 'Italian') & (df['first_course'] == 'Lentejas estofadas')]
    print(f"�Y�O 'Crimen': Italianos sirviendo Lentejas: {len(errores_identidad)} casos")

    # 4. Random sample of 5 rows for visual inspection
    print("\n�Y'? MUESTRA DE CONTROL (10 filas aleatorias):")
